...
<div id="main" class="container" role="main">
  <!-- <a style="margin-left: 98%" class="btn-info" href="./reboot.php">Reboot</a> -->
  <!-- DESC Segment -->
  <div class="row first_placeholder  second_placeholder">
      <h1>Optional Setting Interface (MOSI)</h1><hr>
      <?php
           ...
           <div id="demo" class="well collapsible_box">
              ...
              <fieldset>
               <?php
                  ...
                  //$command = 'systemctl status loraserver.service';
                  //$str = shell_exec($command.' 2>&1 > out.log');
                  //print_r($str);
                  if(isset($log->{"gateway_conf"}->{"server_address"})){
                    ...
                    }elseif($log->{"gateway_conf"}->{"server_address"}=="localhost"){
                      echo '<ul class="btn btn-lg btn-success" style="width:80%;"><li><i class="icon-star"></i> Isolated Gateway is running</li></ul>';
                    }elseif($log->{"gateway_conf"}->{"server_address"}!=""){
                      echo '<ul class="btn btn-lg btn-success" style="width:80%;"><li><i class="icon-star"></i> local community network is running on :'.$log->{"gateway_conf"}->{"server_address"}.'</li></ul>';
                    }
                  }
              ?>
              </fieldset>
              ...
              <form method="post" action="./controller/global_configController.php">
                  <h1 style="text-align:left;"><input type="radio" name="option" value="ttn"/> &nbsp;&nbsp; THE THINK NETWORK COMMUNITY (Optional)</h1>
                  <h1 style="text-align:left;"><input type="radio" name="option" value="isolgate"/> &nbsp;&nbsp;&nbsp;&nbsp; ISOLATED GATEWAY (Optional)</h1>
                  <h1 style="text-align:left;"><input type="radio" name="option" value="lcn"/> &nbsp;&nbsp; LOCAL COMMUNITY NETWORK (LCN)(Optional)</h1>
                  <fieldset>
                    <label class="big">If you choose L.C.N., you have to add :</label>
                    <input type="text" class="span5" name="ip" placeholder="LOCAL SERVER IP ADDRESS ex:192.168.1.230"><br/>
                  </fieldset><br>
